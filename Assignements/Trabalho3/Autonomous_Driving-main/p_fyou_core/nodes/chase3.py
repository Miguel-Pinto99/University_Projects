...
class TurtleBot():
    ...
    def __init__(self):
        ...
        if name in names_red:
            ...
            self.Limits_hunter = [(10,0,0),(255, 0, 0)]
        ...
        print("My name is " + name + ". I am team " + str(self.myteam) + " huntig " + str(
    ...
    def laser(self, msg):
        ...
        if msg.ranges[0]< lim or msg.ranges[345]<lim or msg.ranges[330]<lim or msg.ranges[315]<lim:
            self.wall_R=True
            self.wall_L = False

        elif msg.ranges[1] < lim or msg.ranges[15]<lim or msg.ranges[30]<lim or msg.ranges[45]<lim or msg.ranges[60]<lim:
            self.wall_L=True
            self.wall_R = False
    ...
    def chasing(self,center_x,chase_forward, image_width,flee,center_x_flee):
        ...
        if chase_forward ==True and flee==False:
            ...
            rospy.loginfo("chase")
        #------------Fugir------------------------------
        elif flee==True and wall_R==False and wall_L==False:
            vel_msg.linear.x = 1 # change constant value to change linear velocity
            ...
            rospy.loginfo("flee")
        #---------------Dont' see anything-----------------------------
        elif chase_forward ==False and flee==False and wall_R==False and wall_L==False:
            vel_msg.linear.x=1.1
            vel_msg.angular.z = 0
            self.velocity_publisher.publish(vel_msg)
            rospy.loginfo("nothing")

        #-----------Wall Right------------ Direita toma predominâcia
        elif (wall_R==True and chase_forward==False) or (wall_R==True and wall_L==True and chase_forward==False):
            vel_msg.linear.x = 0.62
            vel_msg.angular.z = 1          #               1
            self.velocity_publisher.publish(vel_msg)
            rospy.loginfo("Right")
        elif wall_L==True and wall_R==False and chase_forward==False:
            vel_msg.linear.x = 0.62
            vel_msg.angular.z = -1       #               1
            self.velocity_publisher.publish(vel_msg)
            rospy.loginfo("Left")
...
def main():
    ...
    #player = TurtleBot(name=rospy.get_name().strip('/'))
    player = TurtleBot()
    rospy.spin()
